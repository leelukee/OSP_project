...
def web_crawling(url):
	...
	#es = Elasticsearch([{'host':elasticsearch_host, 'port':elasticsearch_port}], timeout=10)
	...
	es.index(index="OSP_project", doc_type="bow", id=1, body=doc)
...
def crawl_url():
	if request.method == 'POST':
		...
		res = es.get(index="OSP_project", doc_type="bow", id=1)
		
		d = {}
		
		d['words'] = res['_source']['words']
		d['count'] = res['_source']['count']
		d['count2'] = res['_source']['count']
